...
function HotfixCodeGenHandler.Do(handler, codeGenConfig)
    ...
    local textGetChild = '%s = new %s();'
    local textGetChildAt = "%s = new %s();"
    local textGetChildAs = '%s = m_Comp.GetChild("%s")%s;'
    local textGetChildAtAs = "%s = m_Comp.GetChildAt(%s)%s;"
    local textResetChild = '%s.ResetItem(m_Comp.GetChild("%s"));'

    --- 从自定义配置中读取路径和命名空间
    local exportCodePath = settings.codePath .. "/" .. codePkgName
    fprint("export code path".. exportCodePath)
    ...
    fprint(codePkgName .. "   找到" .. classes.Count .. "个classes的数量")
    ...
    for m = 0, classCntSub - 1 do
        if (m ~= mainPanelIndex) then
            ...
            for n = 0, memberCntSub - 1 do
                ...
                -- fprint("krma............" .. typeNameSub .. " " .. memberInfoSub.varName)
    ...
    for index, value in pairs(pkgNeedGen) do
        fprint("Warning!!! 引用了" .. index .. "包中的组件，" .. " 需要导出" .. index .. "包。")
    end
    -- end
end
